    ...
    methods: {
      ...
      async getData () {
        ...
        let { data } = await axios({
          ...
          method: 'post',
          url: `http://localhost:8080/api/researchproject/getAllResearchproject`,
          data:{
            "id": id,
            "host": this.host,
            "page": this.page
      ...
      toPage(ev){
        this.$router.push('/kejiresult/project/projectEdit?id=' + ev.id)
      },
      dateFormat(row, column, cellValue) {
        return cellValue ? fecha.format(new Date(cellValue), 'YYYY-MM-DD') : '';
      },
      toAddPage(ev){
        this.$router.push("/kejiresult/project/projectAdd")
      },
      getId(ev){
        this.delid = ev;
      ...
      goEdit() {
        this.$router.push('/kejiresult/project/projectEdit?id=' + this.id)
      },
      handleClose () {
        this.form = {
          name: '',
