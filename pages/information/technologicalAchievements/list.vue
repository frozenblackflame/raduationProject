<template>
  <center>
    ...
    <div style="width: 80%;margin-top: 20px;background-color: #cac6c6">
      <el-container>
        <el-header height>
          <el-page-header @back="goBack" content="科技成果列表"></el-page-header>
        </el-header>
        <el-main>
          <label style="margin-left: 700px">成果名称：</label><el-input style="width: 15%;" v-model="title"></el-input>
          <el-button type="primary" @click="getData">搜索</el-button>
          <div>
            <p>{{id}}</p>
            <el-table :data="listDate" stripe style="margin-top: 20px">

              <el-table-column prop="resultsTitle" width="500px" label="标题" show-overflow-tooltip>
              </el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      toPage(ev){
        this.$router.push("/information/technologicalAchievements?id=" +ev.id)
      },
      toAddPage(ev){
        this.$router.push("/article/add?sort=2")
      },
      ...
      importFile(e){
        ...
        console.log(name)
         axios({
          withCredentials: false,
          method: 'post',
          url: `http://localhost:8080/api/news/addResultByExcel`,
          data: formData
        }).then(res =>{
