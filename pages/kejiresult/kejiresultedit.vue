    ...
    methods: {
      async getData(){
        let { data } = await axios({
          ...
          method: 'post',
          url: `http://localhost:8080/api/kejiresult/selectKejiById`,
          data:{
            "id": getQueryString("id")
          }
        })
        this.name = data.extend.results.reultName
        this.desc = data.extend.results.resultDesc;
      ...
      async editResults() {
        ...
        axios({
          withCredentials: false,
          method: 'post',
          url: `http://localhost:8080/api/kejiresult/updateKeji`,
          data: {
            "id": getQueryString("id"),
          ...
          this.$router.push("/result/research?id=" + getQueryString("id"))
        })
      },
      goBack(){
